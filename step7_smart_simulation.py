...
# --- 1. AGENT AND MODEL DEFINITION (SMART AGENTS - FIXED) ---

class VehicleAgent(ap.Agent):
    """A 'smart' vehicle agent that follows a shortest path."""
    
    def setup(self):
        """Called once by the model when the agent is created."""
        self.pos = None # Initialize pos as None
        self.destination = self.model.random.choice(self.model.graph_nodes)
        self.path = [] # Will hold the list of nodes to follow
        # --- FIX: We do NOT calculate path here, pos is not set yet ---
        # self.calculate_new_path() 

    def calculate_new_path(self):
        """Calculates a new shortest path from current position to destination."""
        
        # Ensure agent has a position before calculating
        if self.pos is None:
            print("ERROR: Agent has no position. Cannot calculate path.")
            self.path = []
            return

        try:
            # Use networkx to find the shortest path (based on distance)
            self.path = nx.shortest_path(self.model.G, source=self.pos, target=self.destination, weight='length')
            self.path.pop(0) # Pop the current node
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            # If no path, get a new destination and try again next step
            self.destination = self.model.random.choice(self.model.graph_nodes)
            self.path = []
    ...
    def step(self):
        ...
        if not self.path:
            # If path is empty, we've arrived or have no path.
            # Get a new destination and path.
            self.destination = self.model.random.choice(self.model.graph_nodes)
            self.calculate_new_path()
...
class CityModel(ap.Model):
    ...
    def setup(self):
        ...
        # 3. Create agents (this runs their 'setup' method)
        ...
        for i, agent in enumerate(self.agents):
            ...
            agent.pos = pos                # 1. Tell agent its position
            self.grid[pos].add(agent)      # 2. Add agent to the grid
            self.agent_positions[agent.id] = pos # 3. Add agent to tracker
            
            # --- THIS IS THE FIX ---
            # Now that agent.pos is set, we can call this.
    ...
    def update(self):
        ...
        # Let's track Agent 0's position
...
if __name__ == "__main__":
    ...
    print("--- Starting Task 7: Smart Agent Simulation (Fixed) ---")
